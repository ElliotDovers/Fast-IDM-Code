...
inla_po <- function(unstructured_data, quad, mesh, pred){
  ...
  time.projmesh_pp <- system.time(assign("unstructured_data_A", inla.spde.make.A(mesh = mesh, loc = as.matrix(unstructured_data[,c("x","y")]))))
  ...
  # Create data stack
  time.stk <- system.time(assign("stk_unstructured_data", inla.stack(data=list(y=y.pp, e = e.pp),
  ...
  time.stk_pred1 <- system.time(assign("stk_pred_response", inla.stack(data=list(y=NA),
                                  effects = list(list(data.frame(interceptB=rep(1,np))), env = pred$env, list(Bnodes=1:spde$n.spde)),
                                  A=list(1,1, pred_data_A),
                                  tag='pred_response')))
  
  # combine the stacks
  time.stk_pred2 <- system.time(assign("stk", inla.stack(stk_unstructured_data, stk_pred_response)))
  
  # fit the model without predictions
  result0 <- inla(y ~ interceptB + env + f(Bnodes, model = spde) - 1,
                 family="poisson",
                 data=inla.stack.data(stk_unstructured_data),
                 control.predictor=list(A=inla.stack.A(stk_unstructured_data), compute=TRUE),
  ...
  return(data.frame(MODEL = "PO", FIT = "INLA", KL = KLdiv, MAE = MAE, #TIME = result$cpu.used[4], ALL_TIME = result$cpu.used[4]))
                    TIME = mesh$timing.init + time.spde[3] + time.projmesh_pp[3] + time.projmesh_quad[3] + time.stk[3] + result0$cpu.used[4],
                    TIME_FINAL_FIT = result0$cpu.used[4],
                    TIME_PRED = time.projmesh_pred[3] + time.stk_pred1[3] + time.stk_pred2[3] + result$cpu.used[4],
                    RHO_PA = NA,
                    RHO_PO = sqrt(8)/exp(result$summary.hyperpar["Theta2", "mean"])
  ))
}