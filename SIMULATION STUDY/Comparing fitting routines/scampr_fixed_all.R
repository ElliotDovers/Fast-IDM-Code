...
scampr_fixed_all <- function(structured_data, unstructured_data, quad, pred, domain.data, fld_dim = 100){
  ...
  bfs <<- simple_basis(sqrt(fld_dim), data = domain.data)
  ...
  # )
  ret_obj <- data.frame(MODEL = c("PA", "PO", "IDM"), FIT = rep("SCAMPR FIXED", 3),
