...
tmp <- dat %>% filter(Model != "PO Only") %>% group_by(scen, env_range, bias_range, sim) %>% summarise(check = sum(is.infinite(KLdiv) | is.na(KLdiv) | is.nan(LL)))
...
tmp <- dat %>% filter(Model != "PA Only") %>% group_by(scen, env_range, bias_range, sim) %>% summarise(check = sum(is.infinite(KLdiv_lambda) | is.na(KLdiv_lambda) | is.nan(LL)))
...
# there are also 3 PA Only models that result in KLdiv mu(s) that are effectively Infinite that are throwing off results
dat[dat$KLdiv > 1e7, ]
...
# finally a single IDM result converged to 
...
dat_po <- dat %>% group_by(scen, env_range, bias_range, sim) %>% summarise(delta_kl = KLdiv[Model == "IDM"] - KLdiv[Model == "PO Only"]) # want to show how we can control bias
dat_po$scen_pa <- sapply(strsplit(dat_po$scen, "_PA", fixed = T), function(x){x[2]})
dat_po$Model <- paste0("IDM - N PO = ", sapply(strsplit(sapply(strsplit(dat_po$scen, "PO", fixed = T), function(x){x[2]}), "_", fixed = T), function(x){x[1]}))
dat_po$b_range <- factor(dat_po$bias_range)
dat_po$Model <- factor(dat_po$Model, levels = c("IDM - N PO = 50", "IDM - N PO = 200", "IDM - N PO = 800"))
dat_po$scen_pa <- factor(dat_po$scen_pa, levels = c(50, 200, 800))
...
tmp <- boxplot(delta_kl ~ scen_pa, data = dat_po)
...
p1 <- ggplot(data = dat_pa %>% filter(scen_pa == 50 & b_range == "19"), aes(x = Model, y = delta_kl, color = Model, fill = Model)) +
  ...
  scale_x_discrete(name = "", labels = c(expression(paste(E*"["*n[PO]*"]", " = 50")), expression(paste(E*"["*n[PO]*"]", " = 200")), expression(paste(E*"["*n[PO]*"]", " = 800")))) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        legend.text=element_text(size=10), legend.text.align = 0, legend.position = "none", plot.title = element_text(size = 15),
  ...
  # ggtitle(label = expression(IDM~italic(vs)~PA~only~E*"["*n[PA]*"]"*" = "*50)) +
  labs(title = expression(IDM~~italic(vs.)~~PA~only), subtitle = expression(E*"["*n[PA]*"]"*" = "*50)) +
...
p4 <- ggplot(data = dat_po %>% filter(scen_pa == 50 & b_range == "19"), aes(x = Model, y = delta_kl, color = Model, fill = Model)) +
  geom_boxplot(outlier.shape = 1, outlier.size = 1) +
  scale_x_discrete(name = "", labels = c(expression(paste(E*"["*n[PO]*"]", " = 50")), expression(paste(E*"["*n[PO]*"]", " = 200")), expression(paste(E*"["*n[PO]*"]", " = 800")))) +
  scale_y_continuous(name = "", breaks = c(-3000, -2000, -1000, 0), labels = c("-3K", "-2K", "-1K", "0")) +
  # coord_cartesian(ylim = po_ranges$po50) +
  coord_cartesian(ylim = c(-3000,110)) +
  ...
  # ggtitle(label = expression(paste(E*"["*n[PA]*"]", " = 50"))) +
  labs(title = expression(IDM~~italic(vs.)~~PO~only), subtitle = expression(E*"["*n[PA]*"]"*" = "*50)) +
  geom_abline(intercept = 0, slope = 0, lty = "dashed", color = "darkblue")

p5 <- ggplot(data = dat_po %>% filter(scen_pa == 200 & b_range == "19"), aes(x = Model, y = delta_kl, color = Model, fill = Model)) +
  geom_boxplot(outlier.shape = 1, outlier.size = 1) +
  scale_x_discrete(name = "", labels = c(expression(paste(E*"["*n[PO]*"]", " = 50")), expression(paste(E*"["*n[PO]*"]", " = 200")), expression(paste(E*"["*n[PO]*"]", " = 800")))) +
  scale_y_continuous(name = "", breaks = c(-30000, -20000, -10000, 0), labels = c("-30K", "-20K", "-10K", "0")) +
  # coord_cartesian(ylim = po_ranges$po200) +
  coord_cartesian(ylim = c(-34000,110)) +
  scale_color_manual(values = mod_cols_po) +
  scale_fill_manual(values = alpha(mod_cols_po, alpha = 0.5)) +
...
p6 <- ggplot(data = dat_po %>% filter(scen_pa == 800 & b_range == "19"), aes(x = Model, y = delta_kl, col = Model, fill = Model)) +
  geom_boxplot(outlier.shape = 1, outlier.size = 1) +
  scale_x_discrete(name = "", labels = c(expression(paste(E*"["*n[PO]*"]", " = 50")), expression(paste(E*"["*n[PO]*"]", " = 200")), expression(paste(E*"["*n[PO]*"]", " = 800")))) +
  scale_y_continuous(name = "", breaks = c(-900000, -600000, -300000, 0), labels = c("-900K", "-600K", "-300K", "0")) +
  # coord_cartesian(ylim = po_ranges$po800) +
  coord_cartesian(ylim = c(-1000000,110)) +
  scale_color_manual(values = mod_cols_po) +
  scale_fill_manual(values = alpha(mod_cols_po, alpha = 0.5)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(size = 10),
        panel.background = element_rect(fill = NA, color = "black"), axis.line = element_line(colour = "black"),
