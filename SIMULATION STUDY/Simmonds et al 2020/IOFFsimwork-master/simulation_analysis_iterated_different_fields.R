...
# create some error in the bias covariate
set.seed(1)
bias_error <- rnorm(length(log_mu_A.vec), 0, 1)
set.seed(NULL)

# set up the full quadrature
quad <- data.frame(log_mu_A=log_mu_A.vec,log_lambda_po=log_lambda_po,x=xs,y=ys,env=cov.vec,bias=bias.vec + bias_error,presence=0,quad.size = 1)
...
# add in the bias covariates with error
unstructured_data$bias_old <- unstructured_data$bias
unstructured_data$bias <- quad$bias[match(paste(unstructured_data$x, unstructured_data$y, sep = ":"), paste(quad$x, quad$y, sep = ":"))]
structured_data$bias_old <- structured_data$bias
structured_data$bias <- quad$bias[match(paste(structured_data$x, structured_data$y, sep = ":"), paste(quad$x, quad$y, sep = ":"))]
...
# }
m.c.s <- scampr::scampr(presence ~ env, data = dat.scampr, bias.formula = ~ 1, pa.data = structured_data, basis.functions = bfs, sre.approx = "laplace", model.type = "IDM", latent.po.biasing = F)
...
#   rm(tab)
# }
m.e.s <- scampr::scampr(presence ~ env, data = dat.scampr, bias.formula = ~1, pa.data = structured_data, include.sre = FALSE, model.type = "IDM")
...
save(list = "res_tab", file = paste0(getwd(), "/Results_different_fields/raw/res_", job, ".RDATA"))
