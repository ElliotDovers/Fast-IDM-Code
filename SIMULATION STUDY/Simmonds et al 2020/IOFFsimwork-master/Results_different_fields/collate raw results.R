...
# Check out the number of clearly erroneous convergence (based on MAE)
fit.fails <- dat %>% group_by(Fit, Model) %>% summarise(OK = sum(MAE < 100), FAIL = sum(MAE > 100))
# only a small portion of the IDM with a single shared field

# remove the obsolete joint models
dat <- dat[dat$Fit == "scampr" & dat$Model %in% c('joint', 'jointcov', 'jointtwo'), ]

# create factors from the model type:
dat$Model <- factor(dat$Model,
                    levels = c('jointcov', 'joint', 'jointtwo'),
                    labels = c('IDM (0 flds)', 'IDM (1 flds)', 'IDM (2 flds)')
)

png(filename = paste0(getwd(), "/esa_need_fld.png"), width = 6.2 * plot.res, height = 4.2 * plot.res, res = plot.res)
par(mar = c(0,2.1,0,0))
boxplot(MAE ~ Model, data = dat,
        col = alpha(c("orange", "red", "royalblue"), 0.5), ylim = c(0.3,0.75), main = "", frame = F,
        xaxt = "n", ylab = "", border = c("orange", "red", "royalblue"), xlab = ""
)
dev.off()


ggplot(data = dat %>% filter(Model %in% c('IDM (0 flds)', 'IDM (1 flds)', 'IDM (2 flds)')), aes(y = MAE, fill = Model)) + geom_boxplot() + ylim(mae.lims)
