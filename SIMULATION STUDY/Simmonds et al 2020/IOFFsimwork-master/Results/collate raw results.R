...
mae.lims <- as.vector(unlist(dat0 %>%
  group_by(Model, Fit) %>%
  summarise(ymin = boxplot.stats(MAE)$stats[1], ymax = boxplot.stats(MAE)$stats[5]) %>%
  ungroup() %>%
  summarise(ymin = min(ymin), ymax = max(ymax))
))
kl.lims <- as.vector(unlist(dat0 %>%
  group_by(Model, Fit) %>%
  summarise(ymin = boxplot.stats(KL_div)$stats[1], ymax = boxplot.stats(KL_div)$stats[5]) %>%
  ungroup() %>%
  summarise(ymin = min(ymin), ymax = max(ymax))
))
time.lims <- as.vector(unlist(dat0 %>%
  group_by(Model, Fit) %>%
...
png(filename = paste0(getwd(), "/sym_sim_results_all.png"), width = 6.2 * plot.res, height = 6.2 * plot.res, res = plot.res)
layout(mat = matrix(1:18, nrow = 3, ncol = 6, byrow = TRUE), heights = c(0.375, 0.3, 0.325), widths = c(col1.width, rep((1-col1.width)/5,5)))
par(xpd = T)
for (i in 1:6) {
  if (i == 1) {
