...
genData <- function(dim = c(100,300), 
  ...
  if(plotdat == TRUE){
    par(mfrow=c(1,1)) 
    library(fields) 
    fields::image.plot(list(x=Lam$xcol, y=Lam$yrow, z=t(rf.s)), main='log-Lambda', asp=1) 
    #points(xy, pch=19)
  }
  ...
  # covariate and point process
    ...
    fields::image.plot(list(x=x0, y=y0, z=t(gridcov)), main='Covariate', asp=1)
    fields::image.plot(list(x=x0, y=y0, z=t(rf.s)),
